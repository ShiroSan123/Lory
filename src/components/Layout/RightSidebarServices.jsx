...
function RightSidebarServices({ baseUrl, companyId, service, onSwipeRight }) {
	...
	const handleClick = (params) => {
		onService(params); // Вызываем исходную функцию выбора сервиса
		onSwipeRight(); // Вызываем функцию для свайпа вправо (закрытия панели)
	};
	...
	return (
		<div className="mt-4">
				...
				services.map((serviceItem) => {
					...
					return (
						<div key={serviceItem.id} className="mb-4 rounded-lg">
							<button
								className={`flex text-gray-500 text-lg items-center justify-between w-full p-2 text-left hover:bg-gray-100 rounded-lg border-0 focus:outline-none ${lastClicked.serviceId === serviceItem.id && lastClicked.index === -1
										? 'bg-blue-200'
										: ''
									}`}
								onClick={() => {
									toggleService(serviceItem.id);
									handleClick({ id: serviceItem.id, index: -1 });
									...
									{items.length === 0 ? (
										...
										items.map((item, index) => (
											<div key={index} className="mb-2">
												<button
													className={`text-gray-500 text-lg hover:underline focus:outline-none ${lastClicked.serviceId === serviceItem.id && lastClicked.index === index
															? 'bg-blue-200'
															: ''
														}`}
													onClick={() => {
														handleClick({ id: serviceItem.id, index });
			...
			<div className="mb-4 rounded-lg">
				<button
					className={`flex text-gray-500 text-lg items-center justify-between w-full p-2 text-left hover:bg-gray-100 rounded-lg border-0 bg-blue-200 focus:outline-none ${lastClicked.serviceId === 'custom' ? 'bg-blue-200' : ''
						}`}
					onClick={() => {
						setShowCustomInput((prev) => !prev);
						setLastClicked({ serviceId: 'custom', index: null });
						onSwipeRight(); // Закрываем панель при клике на "Создать услугу"
					}}
				>
