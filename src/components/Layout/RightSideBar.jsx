...
function RightSidebar({ isOpen, onSelectMenu, selectedService, onClose, onOpen }) {
	...
	const [showNotificationModal, setShowNotificationModal] = useState(false); // Состояние для модального окна
	const [notificationClicked, setNotificationClicked] = useState(false); // Флаг первого нажатия
	...
	const handleNotificationClick = () => {
		...
		if (!notificationClicked) {
			setNotificationClicked(true); // Устанавливаем флаг после первого клика
		}
	};
		...
		<>
			...
			{showNotificationModal && (
				<div
					...
					onClick={() => setShowNotificationModal(false)} // Закрытие при клике на фон
				>
					<div
						className={`p-6 rounded-lg shadow-lg ${theme === 'dark' ? 'bg-gray-700 text-white' : 'bg-white text-black'
							}`}
					...
					>
						...
						<button
							...
							className={`mt-4 p-2 rounded-lg ${theme === 'dark' ? 'bg-gray-600 hover:bg-gray-500' : 'bg-gray-200 hover:bg-gray-300'
								}`}
						>
							Закрыть
						</button>
			...
			>
				...
				<div
					className={`p-4 border-t shrink-0 pt-0 ${theme === 'dark' ? 'border-gray-600' : 'border-gray-200'
						}`}
				>
					<div className="mt-6 mb-6">
						...
						>
							...
							<span
								className={`text-lg pb-[2px] font-semibold ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'
									}`}
							>
								LoryAI
							</span>
						...
						<div
							onClick={handleNotificationClick} // Обработчик для уведомлений
							className={`flex items-center p-2 gap-4 rounded-lg w-full text-left select-none cursor-pointer ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-gray-100'
								}`}
						>
							<img src="/ico/bell.svg" alt="Уведомления" className="w-5 h-5" />
							<span
								className={`text-lg font-semibold pb-[2px] ${theme === 'dark' ? 'text-gray-300' : 'text-gray-500'
									}`}
							>
								Уведомления
							</span>
